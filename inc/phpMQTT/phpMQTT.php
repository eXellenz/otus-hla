<?php
/*
	phpMQTT | ANSI | 1.1.0 | 2024.02.29

	Andrew Milsted | andrew@bluerhinos.co.uk | https://github.com/bluerhinos/phpMQTT
	Skurat Denis fork - downgrading supporting php version to 5+ (tested on 5.6.40, 7.1.33, 8.3.2)
...
class phpMQTT
{
	...
	public				$exception		= true;		// proccessing errors like exceptions
	public				$error			= '';		// string buffer for error message
	...
	*/
	public function connect($clean = true, $will = null, $username = null, $password = null)
	{
		...
		if ($this->will !== null)
		{
			...
			if ($this->will['retain'])				// Set will retain
			{
				$var	+= 32;
			}
		}

		if ($this->username !== null)	// Add username to header
		{
			$var	+= 128;
		}

		if ($this->password !== null)	// Add password to header
		{
			$var	+= 64;
		}
	...
	*/
	public function message($msg)
	{
		...
		{
			$strReplace	= str_replace('$', '\$', $key);
			$strReplace	= str_replace('/', "\/", $strReplace);
			$strReplace	= str_replace('+', "[^\/]*", $strReplace);
			$strReplace	= str_replace('#', '.*', $strReplace);
			if (preg_match('/^' . $strReplace . '$/', $topic))
			{
				$found = true;
	...
	/**	The processing loop for an "always on" client set true when you are doing other stuff in the loop good for watching something else at the same time
		@param bool $loop
		@return bool | string
	*/
		...
		else
		{
			...
			$this->_debugMessage(
				sprintf(
					'Received CMD: %d (%s)',
					$cmd,
					(isset($this::$commands[$cmd]) === true) ? ($this::$commands[$cmd]) : ('Unknown')
				)
			);
	...
	*/
	protected function _errorMessage($message)
	{
		...
		{
			$this->error	= 'MQTT Error:' . $message;

			$this->_debugMessage($this->error);
		}
	}
}
